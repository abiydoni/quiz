...
<body class="bg-gradient-to-br from-purple-100 to-blue-200 min-h-screen flex items-center justify-center">
  <div class="w-full max-w-xl bg-white p-8 rounded-2xl shadow-2xl fade-in text-center">
    <h2 class="text-xl font-semibold mb-2">Selamat datang, <span class="text-indigo-700 font-bold"><?= htmlspecialchars($nama_peserta) ?></span></h2>
    <h1 id="soal" class="text-2xl font-bold text-purple-700 mb-6">Menunggu soal...</h1>
    ...
    <div class="grid grid-cols-2 gap-4" id="opsi-jawaban">
      <?php foreach (['A', 'B', 'C', 'D'] as $h): ?>
        <button id="btn_<?= $h ?>" onclick="kirimJawaban('<?= $h ?>')"
          class="bg-purple-600 hover:bg-purple-700 text-white py-3 rounded-lg text-lg font-bold disabled:opacity-50 disabled:cursor-not-allowed transition-all">
          <span id="opsi_<?= $h ?>">Jawaban <?= $h ?></span>
        </button>
      <?php endforeach; ?>
    </div>
  </div>
  <script>
    ...
    async function ambilSoal() {
      const res = await fetch(`api/quiz.php?action=soal_aktif&kode=<?= $kode_quiz ?>`);
      ...
      if (data && data.id_soal && data.id_soal != soalAktif) {
    ...
    async function kirimJawaban(pilihan) {
      ...
      if (data.status === 'ok') {
        Swal.fire({
          icon: data.benar == 1 ? 'success' : 'error',
          title: data.benar == 1 ? '✅ Jawaban BENAR!' : '❌ Jawaban SALAH!',
          text: `Jawaban yang benar: ${data.jawaban_benar}`,
          confirmButtonColor: data.benar == 1 ? '#22c55e' : '#ef4444'
        });
      } else {
        Swal.fire({
          icon: 'error',
